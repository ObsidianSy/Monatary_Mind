...
import { useFinanceiroClient, usePostEvent, useFinanceiroRead } from "@/hooks/useFinanceiro";
...
export default function Categorias() {
  ...
  // Buscar transações do mês atual para calcular totais
  const now = new Date();
  const { data: transacoesDoMes } = useFinanceiroRead<any>(
    client,
    "transacao",
    {
      // Removendo filtro de data temporariamente para buscar TODAS as transações
      // from: startOfMonth(now).toISOString().split('T')[0],
      // to: endOfMonth(now).toISOString().split('T')[0],
      // Não enviar status para buscar TODAS (previsto + liquidado)
    },
    [] // Buscar sempre, sem dependência de mês
  );

  // Debug: Log transações recebidas
  useEffect(() => {
    console.log('📊 Categorias - Transações do mês:', transacoesDoMes?.length || 0);
    if (transacoesDoMes && transacoesDoMes.length > 0) {
      console.log('📊 TODAS as transações:');
      transacoesDoMes.forEach((t: any, idx: number) => {
        console.log(`  [${idx + 1}] ${t.descricao} - R$ ${t.valor}`);
        console.log(`      categoria_id: ${t.categoria_id || 'SEM CATEGORIA'}`);
        console.log(`      categoria_nome: ${t.categoria_nome || 'N/A'}`);
        console.log(`      categoria_parent_id: ${t.categoria_parent_id || 'N/A (categoria pai)'}`);
  ...
  // Calcular totais e contagem por categoria usando transações do mês
  const { categoryTotals, categoryCounts } = useMemo(() => {
    ...
    if (transacoesDoMes && transacoesDoMes.length > 0) {
      ...
      transacoesDoMes.forEach((transacao: any) => {
        const categoriaId = transacao.categoria_id;

        if (!categoriaId) {
          console.warn(`⚠️ Transação "${transacao.descricao}" sem categoria_id!`);
          return;
        }
        ...
        // Somar no total da categoria (seja ela pai ou subcategoria)
        totals[categoriaId] = (totals[categoriaId] || 0) + valorAbsoluto;
        counts[categoriaId] = (counts[categoriaId] || 0) + 1;

        console.log(`  ✅ ${transacao.descricao}: R$ ${valorAbsoluto.toFixed(2)} → categoria ${categoriaId.substring(0, 8)}... (${transacao.categoria_nome})`);
      });
    }

    console.log('💰 Categorias - Totais calculados:', totals);
    console.log('💰 Categorias - Contagens calculadas:', counts);
    console.log('💰 Categorias - Total de categorias com valores:', Object.keys(totals).length);
  ...
  const totalExpenses = categoryTree
    ...
    .reduce((sum, cat) => {
      ...
      const subTotal = cat.subcategorias.reduce((subSum, sub) =>
        subSum + (categoryTotals[sub.id] || 0), 0);
            ...
            {filteredCategories.map((category, categoryIndex) => {
              ...
              return (
                <div key={category.id}>
                  {/* Category Header */}
                  <div
                    className={`p-4 hover:bg-muted/50 transition-colors cursor-pointer ${categoryIndex !== filteredCategories.length - 1 ? 'border-b border-border' : ''
                      }`}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {(() => {
                              const totalTransacoes = (categoryCounts[category.id] || 0) +
                                category.subcategorias.reduce((sum, sub) => sum + (categoryCounts[sub.id] || 0), 0);
                              return `${totalTransacoes} ${totalTransacoes === 1 ? 'transação' : 'transações'}`;
                            })()}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <div className={`text-lg font-bold ${(category.tipo === "credito" || category.tipo === "receita") ? "text-success" : "text-destructive"
                            }`}>
                            {(category.tipo === "credito" || category.tipo === "receita") ? "+" : "-"}{formatCurrency(
                              (categoryTotals[category.id] || 0) +
                              category.subcategorias.reduce((sum, sub) => sum + (categoryTotals[sub.id] || 0), 0)
                            )}
                          </div>
                  ...
                  {isExpanded && (
                    <div className="bg-muted/30">
                      {category.subcategorias.map((subcategory, subIndex) => (
                        <div
                          key={subcategory.id}
                          className={`p-4 pl-16 hover:bg-muted/50 transition-colors ${subIndex !== category.subcategorias.length - 1 ? 'border-b border-border/50' : ''
                            }`}
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-muted-foreground">
                                {categoryCounts[subcategory.id] || 0} {(categoryCounts[subcategory.id] || 0) === 1 ? 'transação' : 'transações'}
                              </div>
                            </div>

                            <div className="flex items-center gap-4">
                              <div className="text-right">
                                <div className={`font-semibold ${(category.tipo === "credito" || category.tipo === "receita") ? "text-success" : "text-destructive"
                                  }`}>
                                  {(category.tipo === "credito" || category.tipo === "receita") ? "+" : "-"}{formatCurrency(categoryTotals[subcategory.id] || 0)}
                                </div>
                              </div>
